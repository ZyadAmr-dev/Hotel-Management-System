...
def create_room(room_number: int, room_type: str, price: float) -> None:
    ...
    if any(map(lambda room: room['roomNumber'] == room_number, rooms)):
...
def check_room_status(room_number: int) -> str:
    ...
    room = next(filter(lambda room: room['roomNumber'] == room_number, rooms), None)
...
def get_customer_info(name: str, contact: str, payment_method: str) -> None:
    ...
    if any(map(lambda customer: customer["name"] == name and customer["contact"] == contact, customers)):
...
def book_room(customer_name: str, room_number: int, check_in_date: str, stay_length: int, contact: str, payment_method: str) -> None:
    ...
    available_room = next(filter(lambda room: room["roomNumber"] == room_number and room["available"], rooms), None)
    
    if available_room:
        available_room["available"] = False
...
def check_in(room_number: int) -> None:
    ...
    reservation = next(filter(lambda res: res["roomNumber"] == room_number and datetime.strptime(res["checkInDate"], "%Y-%m-%d").date() == datetime.now().date(), reservations), None)
...
def check_out(room_number: int) -> None:
    ...
    if room:
        room["available"] = True
        data["reservations"] = list(filter(lambda res: res["roomNumber"] != room_number, reservations))
        save_data(data)
...
def search_on_customers(customer_name: str) -> List[Dict[str, str]]:
    ...
    result = list(filter(lambda customer: customer['name'] == customer_name, customers))
    if result:
...
def calculate_bill(reservation_id: int, tax_rate: float = 0.15, discount: float = 0.2) -> None:
    ...
    reservation = next(filter(lambda res: res["reservationId"] == reservation_id, reservations), None)
    if not reservation:
...
def get_room_occupancy_rate() -> float:
    ...
    occupied_rooms = len(list(filter(lambda room: not room["available"], data["rooms"])))
...
def get_revenue_report(start_date: Optional[str] = None, end_date: Optional[str] = None) -> float:
    ...
    relevant_reservations = filter(
        lambda res: start_date <= datetime.strptime(res["checkInDate"], "%Y-%m-%d") <= end_date,
        reservations
    )

    total_revenue = sum(
        (datetime.strptime(reservation["checkOutDate"], "%Y-%m-%d") - datetime.strptime(reservation["checkInDate"], "%Y-%m-%d")).days * 
        next(room["price"] for room in data["rooms"] if room["roomNumber"] == reservation["roomNumber"])
...
if __name__ == "__main__":
    create_room(1201, "Single", 100)
    create_room(1032, "Double", 150)
    create_room(1013, "Suite", 250)
    create_room(1014, "Single", 100)
    create_room(1505, "Suite", 200)
